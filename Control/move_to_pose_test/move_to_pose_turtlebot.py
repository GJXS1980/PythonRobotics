...
class PathFinderController:
    ...
    Parameters
    ----------
    Kp_rho(线性速度增益) : The linear velocity gain to translate the robot along a line
             towards the goal
    Kp_alpha(角速度增益) : The angular velocity gain to rotate the robot towards the goal
    Kp_beta(补偿角速度增益) : The offset angular velocity gain accounting for smooth merging to
    ...
    def calc_control_command(self, x_diff, y_diff, theta, theta_goal):
        ...
        x_diff : The position of target with respect to current robot position
                 in x direction(目标相对于x方向上当前机器人位置的位置)
        y_diff : The position of target with respect to current robot position
                 in y direction(目标相对于y方向上当前机器人位置的位置)
        theta : The current heading angle of robot with respect to x axis(机器人相对于x轴的当前航向角)
        theta_goal: The target angle of robot with respect to x axis(机器人相对于x轴的目标角度)

        Returns
        -------
        rho : The distance between the robot and the goal position(机器人与目标位置之间的距离)
        v : Command linear velocity(线速度)
        w : Command angular velocity(角速度)
        """
        ...
        rho = np.hypot(x_diff, y_diff)  # 计算直角三角形的斜边
        alpha = (np.arctan2(y_diff, x_diff) 
                 - theta + np.pi) % (2 * np.pi) - np.pi # 利用反正切计算直角三角形的斜角
