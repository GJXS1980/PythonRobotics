...
class CmdVel:
    def __init__(self):
        ...
        self.x0, self.y0, self.angle_y0 = 0, 0, 0
    ...
    def odom_callback(self, data):
        if self.i == 0:
            ...
            self.qx0 = data.pose.pose.orientation.x
            self.qy0 = data.pose.pose.orientation.y
            self.qz0 = data.pose.pose.orientation.z
            self.qw0 = data.pose.pose.orientation.w
            #   四元数转欧拉角
            self.angle_r0, self.angle_p0, self.angle_y0 = tf.transformations.euler_from_quaternion([self.qx0, self.qy0, self.qz0, self.qw0])        
        else:
    ...
    def move_to_pose(self, x_start, y_start, theta_start, x_goal, y_goal, theta_goal):
        ...
        while rho > 0.01 or (abs(self.x - x_goal) > 0.01 and abs(self.y - y_goal) > 0.01):
            ...
            theta = self.angle_y - self.angle_y0
            x = self.x - self.x0
            y = self.y - self.y0
