...
双向 A* 搜索算法是一种优化的 A* 算法，它从起点和终点两个方向同时进行搜索，直到两个搜索路径相交。相比于单向 A* 搜索算法，
双向 A* 算法可以减少搜索的空间和时间复杂度，从而更快地找到最短路径。

a_star_searching_from_two_side 函数接受以下参数：
start: 起点的坐标
goal: 终点的坐标
obstacle_list: 障碍物列表，每个障碍物由一个矩形的左下角和右上角坐标表示
robot_radius: 机器人半径
motion_model: 运动模型，即机器人可以采取的运动方式，比如沿着直线移动或者在圆周上转动等等
max_iter: 最大迭代次数
plot: 是否绘制搜索结果
函数返回一个包含最短路径的坐标列表，以及从起点到终点的总代价（即路径长度）。

总的来说，a_star_searching_from_two_side 函数实现了一个双向 A* 搜索算法，用于在二维地图中寻找起点和终点之间的最短路径。
"""
